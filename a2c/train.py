...
from parl.env.atari_wrappers import MonitorEnv, wrap_deepmind
...
class Learner(object):
    def __init__(self, config):
        ...
        #=========== Create Agent ==========
        env = Environment(config['env_data'])
        env = MonitorEnv(env)
        ...
        model = Model(obs_shape[0],act_dim)
    ...
    def create_actors(self):
        ...
        """
        parl.connect(self.config['master_address'],distributed_files=["../data/u20.txt"])
        self.remote_actors = [
...
if __name__ == '__main__':
    ...
    while not learner.should_stop():
        ...
        while time.time() - start < config['log_metrics_interval_s']:
            learner.step()
            if learner.sample_total_steps % 100000 == 0:
                eval_reward,times = run_evaluate_episodes(learner.agent,env)
                logger.info('Test reward:{} run times:{}'.format(eval_reward,times))
        learner.log_metrics()
